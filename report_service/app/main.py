...
def create_project(project: ProjectCreateSchema, db: Session = Depends(get_db)):
    db_project = Project(
        ...
        static_tests=[],  # Initialize with an empty list
        infiltration_tests=[],  # Initialize with an empty list
        missile_impact_tests=[],  # Initialize with an empty list
        cyclic_tests=[],  # Initialize with an empty list
    )
    db.add(db_project)
    db.commit()
